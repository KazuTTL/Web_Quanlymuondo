...
-- =====================================================
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'Faculties') AND type = 'U')
BEGIN
...
-- 2. Classes (L?p h?c)
-- =====================================================
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'Classes') AND type = 'U')
BEGIN
...
-- 3. Students (Th?ng tin chi ti?t sinh vi?n)
-- =====================================================
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'Students') AND type = 'U')
BEGIN
...
-- 4. Suppliers (Nh? cung c?p thi?t b?)
-- =====================================================
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'Suppliers') AND type = 'U')
BEGIN
...
-- 5. DeviceDamages (H? h?ng, m?t m?t thi?t b?)
-- =====================================================
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'DeviceDamages') AND type = 'U')
BEGIN
    CREATE TABLE DeviceDamages
    (
        ...
        CONSTRAINT CK_DeviceDamages_TrangThai CHECK (
            TrangThaiXuLy IN (N'chua_xu_ly', N'dang_xu_ly', N'da_xu_ly', N'khong_xu_ly')
        )
    );
END
GO

-- 6. MaintenanceRecords (L?ch s? b?o tr? thi?t b?)
-- =====================================================
...
-- 7. Fines (Ti?n ph?t)
-- =====================================================
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'Fines') AND type = 'U')
BEGIN
    CREATE TABLE Fines
    (
...
-- 8. Notifications (Th?ng b?o n?i b?)
-- =====================================================
...
BEGIN
    ...
    (
        ...
        CONSTRAINT CK_Notifications_Loai CHECK (
            LoaiThongBao IN (N'duyet', N'tu_choi', N'qua_han', N'nhe_nhac', N'he_thong')
        )
    );
END
GO

-- 9. DeviceReservations (?t tr??c thi?t b?)
-- =====================================================
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'DeviceReservations') AND type = 'U')
BEGIN
...
-- 10. DeviceImages (H?nh ?nh thi?t b?)
-- =====================================================
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'DeviceImages') AND type = 'U')
BEGIN
...
-- 11. BorrowSessions (Phi?n m??n — m??n nhi?u thi?t b? c?ng l?c)
-- =====================================================
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'BorrowSessions') AND type = 'U')
BEGIN
    CREATE TABLE BorrowSessions
    (
...
-- Th?m c?t SessionID v?o BorrowRecords
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('BorrowRecords') AND name = 'SessionID')
BEGIN
    ALTER TABLE BorrowRecords ADD SessionID INT NULL;
    ALTER TABLE BorrowRecords ADD CONSTRAINT FK_BorrowRecords_Sessions 
        FOREIGN KEY (SessionID) REFERENCES BorrowSessions(SessionID);
END
GO
